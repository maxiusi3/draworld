...
const jwksCache = createLocalJWKSet({ keys: [] as any });
let jwksRemote: ReturnType<typeof createLocalJWKSet> | null = null;
async function ensureJwks() {
  if (!jwksRemote) {
    const res = await fetch(OIDC_JWKS);
    const jwks = await res.json();
    jwksRemote = createLocalJWKSet(jwks);
  }
  return jwksRemote;
}

async function verifyBearer(authorization?: string) {
  if (!authorization || !authorization.startsWith('Bearer ')) throw new Error('Unauthorized');
  const token = authorization.slice('Bearer '.length);
  const { payload } = await jwtVerify(token, await ensureJwks(), {
    issuer: OIDC_ISSUER,
    audience: OIDC_AUD,
  });
...
async function issueSTS(uid: string) {
  ...
  const res = await sts.assumeRole(process.env.ASSUME_ROLE_ARN as string, JSON.stringify(policy), 900, 'tonghua-session');
...
async function otsPutVideoTask(taskId: string, attrs: any) {
...
async function otsGetVideoTask(taskId: string) {
  const client = createOTS();
  const params = {
    tableName: 'videos',
    primaryKey: [{ 'tenantId': 'tonghua' }, { 'videoId': taskId }],
  ...
  const toObj: any = {};
  for (const c of row.attributeColumns || []) toObj[c.columnName] = c.columnValue;
  return { videoId: taskId, ...toObj };
}


// 辅助：写入二级索引表 videos_by_user（按 userId+createdAt）
async function otsIndexVideoByUser(userId: string, createdAt: number, videoId: string) {
  const client = createOTS();
  const params = {
    ...
    primaryKey: [
      { 'tenantId': 'tonghua' },
      { 'userId': userId },
      { 'createdAt': TableStore.Long.fromNumber(createdAt) },
      { 'videoId': videoId }
    ],
    attributeColumns: []
  } as any;
...
async function startVideoTask({ inputImageUrl, params, userId }: { inputImageUrl: string; params?: any; userId: string; }) {
  const requestBody = {
  ...
  if (!resp.ok) throw new Error(`DashScope error: ${resp.status} ${text}`);
  const data = JSON.parse(text);
  if (!data.output?.task_id) throw new Error('DashScope response missing task_id');
...
exports.handler = async function (req: any, res: any, context: any) {
  // 兼容 HTTP 触发器请求
  if (req.method === 'OPTIONS') return res.setHeader('Access-Control-Allow-Origin', '*'), res.setHeader('Access-Control-Allow-Headers', 'Authorization, Content-Type'), res.setHeader('Access-Control-Allow-Methods', 'GET,POST,OPTIONS'), res.status(200).send('OK');
  ...
  try {
    ...
    if (url.pathname === '/api/video/list' && req.method === 'GET') {
      ...
      const inclusiveMax = [
        ...
        { 'videoId': TableStore.INF_MAX }
      ] as any;
      const inclusiveMin = [
        { 'tenantId': 'tonghua' },
        { 'userId': userId },
        { 'createdAt': TableStore.INF_MIN },
        { 'videoId': TableStore.INF_MIN }
      ] as any;
      const rangeReq = {
        tableName: 'videos_by_user',
    ...
    if (url.pathname === '/api/video/status' && req.method === 'GET') {
      ...
      if (!row) return res.status(404).set(corsHeaders()).send(JSON.stringify({ error: 'Not Found' }));
      // 如果仍为 PENDING/RUNNING，尝试查询 DashScope 任务状态
      let status = row.status || 'PENDING';
      let resultVideoUrl = row.resultVideoUrl;
      if (status !== 'SUCCEEDED' && status !== 'FAILED') {
        try {
          const r = await fetch(`https://dashscope.aliyuncs.com/api/v1/tasks/${taskId}`, { headers: { 'Authorization': `Bearer ${TONGYI_API_KEY}` } });
