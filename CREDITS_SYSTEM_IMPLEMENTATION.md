...
### 演示模式（当前实现）
- **数据存储**：内存模拟，重启后重置
- **用户识别**：基于 JWT token 的用户 ID
- **积分规则**：完整实现，但数据不持久化
- **适用场景**：功能演示、开发测试

### 生产模式（升级路径）
- **数据存储**：真实的 Supabase 数据库
- **用户识别**：完整的用户认证和授权
- **积分规则**：完整实现，数据持久化
- **适用场景**：正式生产环境
...
- ✅ **后端代发机制**：使用 `/api/credits/transaction` 的 `x-act-as-user` 头部实现演示模式下的跨用户积分发放
...
- ✅ **模式开关策略**：完善的演示/生产模式自动切换机制，支持环境变量配置

### 环境配置说明
详细的演示模式与生产模式配置请参考：**[DEMO_PRODUCTION_MODE.md](./DEMO_PRODUCTION_MODE.md)**
