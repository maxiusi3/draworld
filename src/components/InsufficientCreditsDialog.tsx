...
export const useInsufficientCreditsCheck = () => {
  ...
  const checkCredits = React.useCallback(async (requiredCredits: number) => {
    try {
  ...
  const InsufficientCreditsDialogComponent = React.useCallback(() => {
    const handleSignin = async () => {
      try {
        const { creditsService } = await import('../services/creditsService');
        await creditsService.dailySignin();
        // 签到成功后可以重新检查积分
      } catch (error) {
  ...
  return {
    checkCredits,
    InsufficientCreditsDialog: InsufficientCreditsDialogComponent,
  };
};
